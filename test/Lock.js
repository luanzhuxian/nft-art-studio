  ...
  describe('Withdrawals', function () {
    ...
    describe('Message', function () {
      ...
      it('Should set the right message', async function () {
        const { lock } = await loadFixture(deployOneYearLockFixture);
        // to.emit(lock, 'SetMessage') 寻找 SetMessage 事件
        const msg = '123';
        await expect(lock.setMessage(msg)).to.emit(lock, 'SetMessage').withArgs(msg);
        await expect(await lock.message()).to.equal(msg);
      });
    });
  });
});
